...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_collectible(origin: OriginFor<T>) -> DispatchResult {
			...
			let (collectible_gen_unique_id, color) = Self::generate_unique_id();
	...
	impl<T: Config> Pallet<T> {
		fn generate_unique_id() -> ([u8; 16], Color) {
			// Create randomness
		...
		fn mint(
			...
			OwnerOfCollectibles::<T>::try_append(&owner, collectible.unique_id.clone())
				.map_err(|_| Error::<T>::MaximumCollectiblesOwned)?;

			// Write new collectible to storage and update the count
			CollectibleMap::<T>::insert(unique_id.clone(), collectible);
			CollectiblesCount::<T>::put(new_count);

			// Deposit the "CollectibleCreated" event.
			Self::deposit_event(Event::<T>::CollectibleCreated { collectible: unique_id.clone(), owner: owner.clone() });
		...
		fn do_buy_collectible(
			...
			OwnerOfCollectibles::<T>::insert(&to, to_owned);
			OwnerOfCollectibles::<T>::insert(&from, from_owned);
			Self::deposit_event(Event::TransferSucceeded { from, to, collectible: unique_id.clone() });
			Ok(())
		}
	}
}
